...
import {
  getD3017AuditInfo,
  addD3017AuditInfo,
  updateD3017AuditInfo,
  deleteD3017AuditInfo,
  submitD3017AuditInfo,
  passD3017AuditInfo,
  rejectD3017AuditInfo} from "@/api/report";
...
export default {
  ...
  methods: {
    ...
    handlePost(formData){
       addD3017AuditInfo(formData).then(res=>{
        console.log(res);
     ...
     handleUpdate(formData) {
       updateD3017AuditInfo(formData).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleDelete(row, index) {
      ...
      deleteD3017AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleSubmit(row) {
      ...
      submitD3017AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handlePass(row) {
      ...
      passD3017AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleReject(row) {
      ...
      this.$prompt('请输入驳回原因', {
          ...
          params.reason = value
          rejectD3017AuditInfo(params).then(res=>{
            console.log(res);
            if (res.data.code == '0000') {
              this._getAuditInfo()
              this.$notify({
