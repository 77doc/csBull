...
import {
  getD3015AuditInfo,
  addD3015AuditInfo,
  updateD3015AuditInfo,
  deleteD3015AuditInfo,
  submitD3015AuditInfo,
  passD3015AuditInfo,
  rejectD3015AuditInfo} from "@/api/report";
...
export default {
  ...
  methods: {
    ...
    handlePost(formData){
       addD3015AuditInfo(formData).then(res=>{
        console.log(res);
     ...
     handleUpdate(formData) {
       updateD3015AuditInfo(formData).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleDelete(row, index) {
      ...
      deleteD3015AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleSubmit(row) {
      ...
      submitD3015AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handlePass(row) {
      ...
      passD3015AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleReject(row) {
      ...
      this.$prompt('请输入驳回原因', {
          ...
          params.reason = value
          rejectD3015AuditInfo(params).then(res=>{
            console.log(res);
            if (res.data.code == '0000') {
              this._getAuditInfo()
              this.$notify({
