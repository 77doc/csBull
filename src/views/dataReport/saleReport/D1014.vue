...
import {
  getD1014AuditInfo,
  addD1014AuditInfo,
  updateD1014AuditInfo,
  deleteD1014AuditInfo,
  submitD1014AuditInfo,
  passD1014AuditInfo,
  rejectD1014AuditInfo} from "@/api/report";
...
export default {
  ...
  methods: {
    ...
    handlePost(formData){
       addD1014AuditInfo(formData).then(res=>{
        console.log(res);
     ...
     handleUpdate(formData) {
       updateD1014AuditInfo(formData).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleDelete(row, index) {
      ...
      deleteD1014AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleSubmit(row) {
      ...
      submitD1014AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handlePass(row) {
      ...
      passD1014AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleReject(row) {
      ...
      this.$prompt('请输入驳回原因', {
          ...
          params.reason = value
          rejectD1014AuditInfo(params).then(res=>{
            console.log(res);
            if (res.data.code == '0000') {
              this._getAuditInfo()
              this.$notify({
