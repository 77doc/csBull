...
import {
  getD3002AuditInfo,
  addD3002AuditInfo,
  updateD3002AuditInfo,
  deleteD3002AuditInfo,
  submitD3002AuditInfo,
  passD3002AuditInfo,
  rejectD3002AuditInfo} from "@/api/report";
...
export default {
  ...
  methods: {
    ...
    handlePost(formData){
       addD3002AuditInfo(formData).then(res=>{
        console.log(res);
     ...
     handleUpdate(formData) {
       updateD3002AuditInfo(formData).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleDelete(row, index) {
      ...
      deleteD3002AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleSubmit(row) {
      ...
      submitD3002AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handlePass(row) {
      ...
      passD3002AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleReject(row) {
      ...
      this.$prompt('请输入驳回原因', {
          ...
          params.reason = value
          rejectD3002AuditInfo(params).then(res=>{
            console.log(res);
            if (res.data.code == '0000') {
              this._getAuditInfo()
              this.$notify({
