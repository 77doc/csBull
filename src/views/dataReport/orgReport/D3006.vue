...
import {
  getD3006AuditInfo,
  addD3006AuditInfo,
  updateD3006AuditInfo,
  deleteD3006AuditInfo,
  submitD3006AuditInfo,
  passD3006AuditInfo,
  rejectD3006AuditInfo} from "@/api/report";
...
export default {
  ...
  methods: {
    ...
    handlePost(formData){
       addD3006AuditInfo(formData).then(res=>{
        console.log(res);
     ...
     handleUpdate(formData) {
       updateD3006AuditInfo(formData).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleDelete(row, index) {
      ...
      deleteD3006AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleSubmit(row) {
      ...
      submitD3006AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handlePass(row) {
      ...
      passD3006AuditInfo(params).then(res=>{
        console.log(res);
        if (res.data.code == '0000') {
          this._getAuditInfo()
          this.$notify({
    ...
    handleReject(row) {
      ...
      this.$prompt('请输入驳回原因', {
          ...
          params.reason = value
          rejectD3006AuditInfo(params).then(res=>{
            console.log(res);
            if (res.data.code == '0000') {
              this._getAuditInfo()
              this.$notify({
